...
namespace PdfPigMLNetBlockClassifier.LightGbmV2
{
    public static class LightGbmModelBuilder
    {
        ...
        public static void TrainModel(string trainDataFilePath, string outputModelName)
        {
...
across all the features of a model, one after another.\n");
            ...
            // microaccuracy.
            var sortedIndicesMicro = pfi.Select((metrics, index) => new { index, metrics.MicroAccuracy })
            ...
            foreach (int i in sortedIndicesMicro)
            {
            ...
            // overall. Get the feature indices sorted by their impact on
            // macroaccuracy.
            var sortedIndicesMacro = pfi.Select((metrics, index) => new { index, metrics.MacroAccuracy })
                                   .OrderByDescending(feature => Math.Abs(feature.MacroAccuracy.Mean))
                                   .Select(feature => feature.index);

            Console.WriteLine("Feature\tChange in MacroAccuracy\t95% Confidence in the Mean Change in MacroAccuracy");
            var macroAccuracy = pfi.Select(x => x.MacroAccuracy).ToArray();

            foreach (int i in sortedIndicesMacro)
            {
                Console.WriteLine("{0}\t{1:G4}\t{2:G4}",
                                  featureColumns[i],
                                  macroAccuracy[i].Mean,
                                  1.96 * macroAccuracy[i].StandardError);
            }
        }
